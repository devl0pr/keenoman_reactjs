        ...
        instance.post('/login', formData)
            .then(function (response) {
                ...
                if (response.data.data) {
                    setUserInfo(response.data.data.token, response.data.data.username, response.data.data.email, response.data.data.id)
                    navigate("/")
                }
            })
                                ...
                                <div className="sign__content">
                                    <form action="components/auth/signin#" className="sign__form" onSubmit={onsSubmitHandler}>
                                        ...
                                        <div className="sign__group">
                                            <input name="email" type="email" className="sign__input"
                                                   placeholder="Email" onChange={onInputChangeHandler}/>
                                            { formErrors.email && <p style={{fontSize: '12px',  color: '#df4a32', padding: '10px 0px 0 6px'}}>{formErrors.email}</p> }
                                        </div>
                                        ...
                                        {
                                            loading ? <button className="sign__btn" type="button">Please wait</button> : <button className="sign__btn" type="submit" onSubmit={onsSubmitHandler}>Sign in </button>
                                        }
                                        ...
                                        {/*    <a className="fb" href="#">*/}
                                        {/*        <svg viewBox="0 0 9 17" fill="none"*/}
                                        ...
                                        {/*    href="forgot.html">Forgot password?</a></span>*/}
                                    </form>
                                </div>
                            </div>
                        </div>
